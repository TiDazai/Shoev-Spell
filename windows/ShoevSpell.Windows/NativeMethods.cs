...
internal static class NativeMethods
{
    ...
    internal struct InputUnion { [FieldOffset(0)] internal KeybdInput keyboard; }
    [StructLayout(LayoutKind.Sequential)]
    ...
    internal static void ReplaceText(int deleteUtf16Units, string replacement, string delimiter)
    {
    ...
    private static Input NewKeyboard(ushort key, ushort scan, uint flags) => new()
    {
        type = InputKeyboard,
        data = new InputUnion { keyboard = new KeybdInput { virtualKey = key, scanCode = scan, flags = flags, extraInfo = SelfInputMarker } }
    };
}
