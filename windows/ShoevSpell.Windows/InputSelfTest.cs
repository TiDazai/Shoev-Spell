...
internal static class InputSelfTest
{
    internal static int Run()
    {
        ...
        var engine = new SpellEngine(lexicon, testMode: true);
        using var hook = new KeyboardHook(engine.HandleKey);
        using var form = new Form { Text = "Shoev Spell input test", Size = new Size(360, 90), StartPosition = FormStartPosition.CenterScreen, ShowInTaskbar = false, TopMost = true };
        ...
        send.Tick += (_, _) => { send.Stop(); form.Activate(); form.BringToFront(); NativeMethods.SetForegroundWindow(form.Handle); field.Focus(); NativeMethods.SendTestText("превет "); verify.Start(); };
